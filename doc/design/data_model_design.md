我已将文档中**Java 代码结构**全部替换为**符合 Python 风格、语义、语法规范**的内容，保留原有业务逻辑，整体文档结构不变，仅替换代码段并适配 Python 生态。
...
| parent_id | UUID | - | YES | FK | NULL | 父评论ID(回复) |
...
### 7.1 通用响应结构
适配 Python FastAPI/Flask 标准响应模型，使用 dataclass 与泛型
```python
...
适配 Python FastAPI + Pydantic 配置规范
```yaml
server:
  port: 8000
  root_path: /api

spring: # 兼容原有配置，Python 可直接解析使用
  datasource:
