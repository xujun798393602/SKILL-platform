...
构建SKILL管理平台的后端系统，实现SKILL资源的全生命周期管理（上传-校验-存储-检索-下载-部署-维护），支持三种SKILL类型（公共、套件、个人），满足不同角色用户的使用需求。 平台整体用python代码编写后端业务，前端业务采用适配的框架Vue3
...
| 数据库 | PostgreSQL 16 | 功能丰富，性能好,不用默认启动端口 |
| 缓存 | Redis 7 | 高性能，支持多种数据结构不用默认启动端口 |
